...
public class ReturnCodesXMLFactory {
   private static final String _CLASS=ReturnCodesXMLFactory.class.getName();
   private static final Logger logger=Logger.getLogger(_CLASS);
   private static final String elementLabel="return_codes";
   
   public List<ReturnCode> parse(XMLStreamReader reader) 
         throws XMLStreamException {
         logger.entering(_CLASS, "parse(XMLStreamReader)",reader);
         boolean done=false;
         List<ReturnCode> drivers=null;
         ReturnCode returnCode=null;
         ReturnCodeXMLFactory returnCodeFactory=null;
         String elementName=null;
         String msg=null;
         
         drivers=new ArrayList<ReturnCode>();
         if (reader != null) {
            returnCodeFactory=new ReturnCodeXMLFactory();
            while ((reader.hasNext() && !done)) {
               switch (reader.getEventType()) {
                  case XMLStreamConstants.START_ELEMENT:
                     ...
                     switch(elementName) {
                        case elementLabel:
                           returnCode=returnCodeFactory.parse(reader);
                           if (returnCode != null) {
                              drivers.add(returnCode);                           
         ...
         logger.entering(_CLASS, "parse(XMLStreamReader)",drivers);
         return drivers;
   }
}