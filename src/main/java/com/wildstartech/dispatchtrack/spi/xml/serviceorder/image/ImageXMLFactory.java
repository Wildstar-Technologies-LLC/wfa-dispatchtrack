...
import com.wildstartech.dispatchtrack.spi.ImageImpl;
...
public class ImageXMLFactory extends XMLFactory<Image> {
   private static final String _CLASS=ImageXMLFactory.class.getName();
    ...
    */
   public ImageXMLFactory() {
      logger.entering(_CLASS, "ImageXMLFactory()");
      registerXMLElementProcessor(new ImageElementProcessor());
      registerXMLElementProcessor(new SrcElementProcessor());
      registerXMLElementProcessor(new ThumbnailElementProcessor());
      registerXMLElementProcessor(new CreatedAtElementProcessor());      
      logger.exiting(_CLASS, "ImageXMLFactory()");
   }
   
   @Override
   public Image parse(XMLStreamReader reader) throws XMLStreamException {
      logger.entering(_CLASS, "parse(XMLStreamReader)",reader);
      boolean done=false;
      Image image=null;
      String elementName=null;
      String msg=null;
      XMLElementProcessor<Image> handler=null;
            ...
            while ((reader.hasNext()) && !done) {
               switch (reader.getEventType()) {
                  case XMLStreamConstants.START_ELEMENT:
                     ...
                     if (handler != null) {
                        handler.processElement(image, reader);
                        handler=null;
                     } // END if (handler != null)
                     break;
      ...
      logger.exiting(_CLASS, "parse(XMLStreamReader)",image);
      return image;
   }
}