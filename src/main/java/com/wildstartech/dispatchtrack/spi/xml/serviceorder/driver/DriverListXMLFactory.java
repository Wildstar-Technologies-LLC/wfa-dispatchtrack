...
public class DriverListXMLFactory {
   private static final String _CLASS=DriverListXMLFactory.class.getName();
   private static final Logger logger=Logger.getLogger(_CLASS);
   
   public List<Driver> parse(XMLStreamReader reader) 
         throws XMLStreamException {
         logger.entering(_CLASS, "parse(XMLStreamReader)",reader);
         boolean done=false;
         List<Driver> drivers=null;
         Driver driver=null;
         DriverXMLFactory driverFactory=null;
         String elementName=null;
         String msg=null;
         
         drivers=new ArrayList<Driver>();
         if (reader != null) {
            driverFactory=new DriverXMLFactory();
            while ((reader.hasNext() && !done)) {
               switch (reader.getEventType()) {
                  case XMLStreamConstants.START_ELEMENT:
                     ...
                     switch(elementName) {
                        case "driver":
                           driver=driverFactory.parse(reader);
                           if (driver != null) {
                              drivers.add(driver);                           
