...
import com.wildstartech.dispatchtrack.spi.NoteImpl;
...
public class NoteXMLFactory extends XMLFactory<Note> {
   private static final String _CLASS=NoteXMLFactory.class.getName();
   private static final Logger logger=Logger.getLogger(_CLASS);
   private static final String elementLabel="note";
   
   /** 
    * Default, no-argument constructor.
    */
   public NoteXMLFactory() {
      logger.entering(_CLASS, "ShipmentEventXMLFactory()");
      logger.exiting(_CLASS, "ShipmentEventXMLFactory()");
   }
   
   @Override
   public Note parse(XMLStreamReader reader) throws XMLStreamException {
      logger.entering(_CLASS, "parse(XMLStreamReader)",reader);
      boolean done=false;
      Note note=null;
      String elementName=null;
      String msg=null;
      XMLElementProcessor<Note> handler=null;
            ...
            while ((reader.hasNext()) && !done) {
               switch (reader.getEventType()) {
                  case XMLStreamConstants.START_ELEMENT:
                     ...
                     if (handler != null) {
                        handler.processElement(note, reader);
                        handler=null;
                     } // END if (handler != null)
                     break;
      ...
      logger.exiting(_CLASS, "parse(XMLStreamReader)",note);
      return note;
   }
}