...
/**
 * Creates a list of {@code ServiceOrder} objects from the XML stream.
 * 
 * @author Derek Berube, Wildstar Technologies, LLC.
 * @version 0.1, 2017.05.03
 */
public class ServiceOrdersXMLFactory {
   private static final String _CLASS=
         ServiceOrdersXMLFactory.class.getName();
   private static final Logger logger=Logger.getLogger(_CLASS);
   private static final String elementLabel="service_order";
   private static final String listElementLabel="service_orders";
   
   public List<ServiceOrder> parse(XMLStreamReader reader) 
      throws XMLStreamException {
      logger.entering(_CLASS, "parse(XMLStreamReader)",reader);
      boolean done=false;
      List<ServiceOrder> serviceOrders=null;
      ServiceOrder serviceOrder=null;
      ServiceOrderXMLFactory soFactory=null;
      String elementName=null;
      String msg=null;
      
      serviceOrders=new ArrayList<ServiceOrder>();
      if (reader != null) {
         soFactory=new ServiceOrderXMLFactory();
         while ((reader.hasNext() && !done)) {
            switch (reader.getEventType()) {
               case XMLStreamConstants.START_ELEMENT:
                  ...
                  switch(elementName) {
                     case elementLabel:
                        serviceOrder=soFactory.parse(reader);
      ...
      logger.entering(_CLASS, "parse(XMLStreamReader)",serviceOrders);
