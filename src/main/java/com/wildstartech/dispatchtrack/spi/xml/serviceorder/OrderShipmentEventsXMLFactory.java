...
import com.wildstartech.dispatchtrack.spi.xml.serviceorder.shipmentevent.ShipmentEventXMLFactory;

public class OrderShipmentEventsXMLFactory {
   private static String _CLASS=
         OrderShipmentEventsXMLFactory.class.getName();
   private static Logger logger=Logger.getLogger(_CLASS);
   private static final String elementLabel="shipment_event";
   
   public List<ShipmentEvent> parse(XMLStreamReader reader) 
   throws XMLStreamException {
      logger.entering(_CLASS, "parse(XMLStreamReader)",reader);
      boolean done=false;
      List<ShipmentEvent> shipmentEvents=null;
      ShipmentEvent event=null;
      ShipmentEventXMLFactory shipmentEventFactory=null;
      String elementName=null;
      String msg=null;
      
      shipmentEvents=new ArrayList<ShipmentEvent>();
      if (reader != null) {
         shipmentEventFactory=new ShipmentEventXMLFactory();
         while ((reader.hasNext() && !done)) {
            switch (reader.getEventType()) {
               case XMLStreamConstants.START_ELEMENT:
                  ...
                  switch(elementName) {
                     case elementLabel:
                        event=shipmentEventFactory.parse(reader);
                        if (event != null) {
      ...
      logger.entering(_CLASS, "parse(XMLStreamReader)",shipmentEvents);
