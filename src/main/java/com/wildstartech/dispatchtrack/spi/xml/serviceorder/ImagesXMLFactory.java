...
import com.wildstartech.dispatchtrack.spi.xml.serviceorder.image.ImageXMLFactory;

public class ImagesXMLFactory {
   private static final String _CLASS=ImagesXMLFactory.class.getName();
   private static final Logger logger=Logger.getLogger(_CLASS);
   private static final String elementLabel="image";
   
   public List<Image> parse(XMLStreamReader reader) 
         throws XMLStreamException {
         logger.entering(_CLASS, "parse(XMLStreamReader)",reader);
         boolean done=false;
         List<Image> images=null;
         Image image=null;
         ImageXMLFactory imageFactory=null;
         String elementName=null;
         String msg=null;
         
         images=new ArrayList<Image>();
         if (reader != null) {
            imageFactory=new ImageXMLFactory();
            while ((reader.hasNext() && !done)) {
               switch (reader.getEventType()) {
                  case XMLStreamConstants.START_ELEMENT:
                     ...
                     switch(elementName) {
                        case elementLabel:
                           image=imageFactory.parse(reader);
                           if (image != null) {
                              images.add(image);                           
                           } // END if (image != null)
                           break;
         ...
         logger.entering(_CLASS, "parse(XMLStreamReader)",images);
         return images;
   }
}