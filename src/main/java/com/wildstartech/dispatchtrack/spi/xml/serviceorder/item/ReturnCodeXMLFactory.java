...
import com.wildstartech.dispatchtrack.spi.ReturnCodeImpl;
...
public class ReturnCodeXMLFactory extends XMLFactory<ReturnCode> {
   private static final String _CLASS=ReturnCodeXMLFactory.class.getName();
   private static final Logger logger=Logger.getLogger(_CLASS);
   private static final String elementLabel="return_code";
    ...
    */
   public ReturnCodeXMLFactory() {
      logger.entering(_CLASS, "DriverXMLFactory()");
         registerXMLElementProcessor(new ReturnCodeElementProcessor());
      logger.exiting(_CLASS, "DriverXMLFactory()");
   }
   
   @Override
   public ReturnCode parse(XMLStreamReader reader) throws XMLStreamException {
      logger.exiting(_CLASS, "parse(XMLStreamReader)",reader);
      boolean done=false;
      ReturnCode returnCode=null;
      String elementName=null;
      XMLElementProcessor<ReturnCode> handler=null;
            ...
            while ((reader.hasNext()) && !done) {
               switch (reader.getEventType()) {
                  case XMLStreamConstants.START_ELEMENT:
                     ...
                     if (handler != null) {
                        handler.processElement(returnCode, reader);
                        handler=null;
                     } // END if (handler != null)
                     break;
      ...
      logger.exiting(_CLASS, "parse(XMLStreamReader)",returnCode);
      return returnCode;
   }
}