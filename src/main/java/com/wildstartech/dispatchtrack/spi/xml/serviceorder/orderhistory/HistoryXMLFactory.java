...
import com.wildstartech.dispatchtrack.spi.HistoryEventImpl;
...
public class HistoryXMLFactory extends XMLFactory<HistoryEvent> {
   private static final String _CLASS=HistoryXMLFactory.class.getName();
    ...
    */
   public HistoryXMLFactory() {
      logger.entering(_CLASS, "HistoryXMLFactory()");
      registerXMLElementProcessor(new HistoryElementProcessor());
      logger.exiting(_CLASS, "HistoryXMLFactory()");
   }
   
   @Override
   public HistoryEvent parse(XMLStreamReader reader) throws XMLStreamException {
      logger.entering(_CLASS, "parse(XMLStreamReader)",reader);
      boolean done=false;
      HistoryEvent event=null;
      String elementName=null;
      String msg=null;
      XMLElementProcessor<HistoryEvent> handler=null;
            ...
            while ((reader.hasNext()) && !done) {
               switch (reader.getEventType()) {
                  case XMLStreamConstants.START_ELEMENT:
                     ...
                     if (handler != null) {
                        handler.processElement(event, reader);
                        handler=null;
                     } // END if (handler != null)
                     break;
      ...
      logger.exiting(_CLASS, "parse(XMLStreamReader)",event);
      return event;
   }
}