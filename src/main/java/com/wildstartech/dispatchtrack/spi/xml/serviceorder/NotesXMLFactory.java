...
public class NotesXMLFactory {
   private static String _CLASS=
         NotesXMLFactory.class.getName();
   private static Logger logger=Logger.getLogger(_CLASS);
   private static final String elementLabel="notes";
   
   public List<Note> parse(XMLStreamReader reader) 
   throws XMLStreamException {
      logger.entering(_CLASS, "parse(XMLStreamReader)",reader);
      boolean done=false;
      List<Note> shipmentEvents=null;
      Note event=null;
      NoteXMLFactory noteFactory=null;
      String elementName=null;
      String msg=null;
      
      shipmentEvents=new ArrayList<Note>();
      if (reader != null) {
         noteFactory=new NoteXMLFactory();
         while ((reader.hasNext() && !done)) {
            switch (reader.getEventType()) {
               case XMLStreamConstants.START_ELEMENT:
                  ...
                  switch(elementName) {
                     case elementLabel:
                        event=noteFactory.parse(reader);
                        if (event != null) {
