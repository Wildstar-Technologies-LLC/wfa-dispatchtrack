...
import com.wildstartech.dispatchtrack.spi.xml.serviceorder.item.ItemXMLFactory;

public class ItemsXMLFactory {
   private static String _CLASS=
         ItemsXMLFactory.class.getName();
   private static Logger logger=Logger.getLogger(_CLASS);
   private static final String elementLabel="item";
   
   public List<Item> parse(XMLStreamReader reader) 
   throws XMLStreamException {
      logger.entering(_CLASS, "parse(XMLStreamReader)",reader);
      boolean done=false;
      List<Item> items=null;
      Item item=null;
      ItemXMLFactory itemFactory=null;
      String elementName=null;
      String msg=null;
      
      items=new ArrayList<Item>();
      if (reader != null) {
         itemFactory=new ItemXMLFactory();
      ...
      logger.entering(_CLASS, "parse(XMLStreamReader)",items);
      return items;
   }
}