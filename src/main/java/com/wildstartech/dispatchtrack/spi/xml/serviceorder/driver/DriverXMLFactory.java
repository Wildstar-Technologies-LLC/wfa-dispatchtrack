...
import com.wildstartech.dispatchtrack.spi.DriverImpl;
...
public class DriverXMLFactory extends XMLFactory<Driver> {
   private static final String _CLASS=DriverXMLFactory.class.getName();
   private static final Logger logger=Logger.getLogger(_CLASS);
   private static final String elementLabel="driver";
    ...
    */
   public DriverXMLFactory() {
      logger.entering(_CLASS, "DriverXMLFactory()");
         registerXMLElementProcessor(new IdElementProcessor());
         registerXMLElementProcessor(new NameElementProcessor());
      logger.exiting(_CLASS, "DriverXMLFactory()");
   }
   
   @Override
   public Driver parse(XMLStreamReader reader) throws XMLStreamException {
      logger.exiting(_CLASS, "parse(XMLStreamReader)",reader);
      boolean done=false;
      Driver driver=null;
      String elementName=null;
      XMLElementProcessor<Driver> handler=null;
            ...
            while ((reader.hasNext()) && !done) {
               switch (reader.getEventType()) {
                  case XMLStreamConstants.START_ELEMENT:
                     ...
                     if (handler != null) {
                        handler.processElement(driver, reader);
                        handler=null;
                     } // END if (handler != null)
                     break;
      ...
      logger.exiting(_CLASS, "parse(XMLStreamReader)",driver);
      return driver;
   }
}